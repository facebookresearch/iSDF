...
class iSDFWindow:
    ...
    def __init__(self, trainer, optim_iter, font_id):
        ...
        if trainer.incremental or trainer.live:
            ...
            button_clear_kf.vertical_padding_em = 0
    ...
    def _clear_keyframes(self):
        ...
        self.iter = 0
        info, new_kf = self.optim_iter(self.trainer, self.iter)
        self.iter += 1
        self.kfs = [new_kf]
        ...
        while not self.is_done:
            ...
            for i in range(self.training_iters_slider.int_value):
                ...
                if new_kf is not None:
                    if isinstance(new_kf, int):
                        if self.trainer.incremental and not self.trainer.live:
                            self.is_done = True
                    self.kfs.append(new_kf)

                self.output_info.text = f"Step {self.step} -- Iteration {self.iter}\n" + info
                self.iter += 1
            ...
            if self.slices_box.checked and self.sdf_grid_pc is not None:
                ...
                slice_pcd = o3d.t.geometry.PointCloud(np.zeros((1, 3), dtype=np.float32))
